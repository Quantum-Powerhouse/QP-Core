...
- `pet/QuantumPet.tsx`, the character: reacts to **real** quantum events from the
  site-wide event bus (`src/lib/quantum/events.ts` documents what the pet may say:
...
- **The paper** (`/research/paper`): the research record rendered as a citable PDF
  (`paper/` in the research repository, attached to GitHub release v1.1.1), with BibTeX,
  `CITATION.cff`, and the archival status (Zenodo DOI pending the owner's toggle;
  arXiv not submitted).
