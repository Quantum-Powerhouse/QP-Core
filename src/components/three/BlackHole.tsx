...
function AccretionDisk() {
  ...
  const geometry = useMemo(() => {
    ...
    const inner = new THREE.Color("#e8ecf2");
    const mid = new THREE.Color("#9fb4cc");
    const outer = new THREE.Color("#5c728c");
  ...
  const rimMaterial = useMemo(
    () =>
      new THREE.ShaderMaterial({
        ...
        uniforms: {
          color: { value: new THREE.Color("#9fb4cc") },
        },
        vertexShader: `
...
function Scene({ particleScale }: { particleScale: number }) {
  ...
  return (
    <group ref={groupRef}>
      <EventHorizon />
      <AccretionDisk />
      <DustField count={Math.round(900 * particleScale)} innerRadius={3.2} outerRadius={7.5} color="#5c728c" size={0.03} speed={0.02} />
      <DustField count={Math.round(600 * particleScale)} innerRadius={5} outerRadius={9} color="#9fb4cc" size={0.024} speed={-0.014} />
    </group>
  );
}
