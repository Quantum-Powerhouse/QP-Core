  ...
  return (
    <>
      ...
      <p className="mt-4 max-w-2xl text-lg leading-relaxed text-muted">
        Same rule as the research wing: nothing is asserted that a reader can&apos;t open. Every card carries a status —
        a published result is not a vendor announcement is not a roadmap is not an opinion — and the date of the
        ...
        <ul className="mt-3 flex flex-col gap-2">
          {HARDWARE_STATE.map((s) => (
            <li key={s.label} className="flex gap-3 text-sm">
              <span className={`font-mono ${s.achieved ? "text-accent" : "text-[#f59e0b]"}`}>{s.achieved ? "✓ achieved" : "✗ not yet"}</span>
