...
export const metadata: Metadata = buildMetadata({
  ...
  description:
    "The real tradeoffs behind Quantum Powerhouse: a from-scratch TypeScript physics engine instead of a JavaScript Qiskit, objects versus typed arrays at the 2ⁿ wall, exact density matrices for noise, parameter-shift gradients, a Python transpiler service (no Rust), evidence-integrity CI, the limits on what the pet may say, and the bugs CI caught that a laptop never would.",
  path: "/engineering",
...
const DECISIONS: { title: string; problem: string; call: string; cost: string; proof: { label: string; href: string } }[] = [
  ...
  },
  {
    ...
    cost: "Two engines to keep in agreement, enforced by a test. The benchmark on the arcade runs both live on the visitor's machine, which shows the 2ⁿ wall instead of describing it.",
    proof: { label: "Engine Scaling Benchmark", href: "/playground/arcade#engine-scaling-benchmark" },
  },
  {
    title: "Exact density matrices for noise",
    ...
    call: "Exact density matrices everywhere noise appears, the ZNE panel, the Decoherence Dial, the Circuit Lab, so purity and coherence are exact values.",
  ...
  },
  {
    ...
    call: "A FastAPI service (Python, Qiskit, qiskit-braket-provider) with a labeled mock on the site when the service is down: the mock's Braket IR carries mock: true, and its circuit metrics are still computed from the real QASM. While writing this page I found an SEO keyword claiming a 'Rust quantum compiler pass' on two pages. There is no Rust in the project, so I removed the keyword.",
    cost: "A second deployment to keep alive, and the site has to say so when it isn't. The real-hardware lane lives in the same service so the whole 'leaves the browser' path is one codebase.",
    proof: { label: "docs: transpiler pipeline", href: "/docs/qp-core/transpiler-pipeline" },
  },
  {
    title: "Evidence-integrity CI",
    ...
    cost: "Adding a claim means touching two files and satisfying a test. That friction is deliberate.",
  ...
  {
    title: "What the pet is allowed to say",
    problem: "A talking companion is a temptation to invent: it could easily 'explain' physics it hasn't computed.",
    call: "QPet can only say three kinds of things: lines about real routes and sections, lines about its own emotional state machine, and narration of real engine events with the numbers read from the event payload. No language model. Its collapses are Born-rule samples.",
    cost: "It is less impressive than a chatbot would be. A pet that invents a Hamiltonian would break the site's one rule, so that is the trade.",
  ...
  },
  {
    ...
    call: "Documented here. Every deploy in the history was a deliberate CLI action after a green CI run; the fix is a one-minute dashboard change that requires the owner's account.",
    cost: "A person sits between 'green' and 'live'. That is stated here instead of pretending the pipeline is automatic.",
    ...
    <div className="flex flex-1 flex-col">
      ...
      <main className="mx-auto w-full max-w-4xl flex-1 px-6 py-16">
        ...
        <p className="mt-4 max-w-2xl text-lg leading-relaxed text-muted">
          This page lists the tradeoffs that shaped the site: the problem, the call, what it cost, and where you can
          check it. One entry records a false claim I found and removed while writing it.
        </p>
        ...
        <p className="mt-10 text-sm text-muted">
          <Link href="/builder" className="text-accent">The builder page</Link> says who made these calls.{" "}
          <Link href="/research/paper" className="text-accent">The paper</Link> is what they led to.
        </p>
      </main>
      <SiteFooter />
    </div>
